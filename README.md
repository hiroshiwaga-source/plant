...
ログイン後は **植物の一覧・追加・編集・削除**、**写真の登録（Storage）**、**AI 状態メモ（Edge `ai-diagnose`・スタブ応答）**、**今日のケア提案（Edge `care-recommendations`・ルール）**、および **水やり／肥料／剪定／植え替えの育成ログ**（任意メモ・相対日時表示つき）が利用できます。AI 本体の外部 API 接続はサーバー側シークレットで後から差し替え可能です。

### 機能の洗い出し（ユーザー目線・プロ目線）

| 区分 | 内容 |
|------|------|
| **今回反映した** | 植物詳細で **写真アップロード**（`plant_photos` → `plant-photos`）、**AI診断・ケア提案ボタン**（`supabase.functions.invoke`）。世話ログの**任意メモ**・**相対日時**。**ログアウト確認**・**VoiceOver ラベル**。`console.warn` は **`__DEV__` のみ**。 |
| **追加を検討（未実装／部分）** | AI の**実モデル接続**（Edge はスタブ／ルール）。**過去日時の世話**（いまは記録時刻＝現在）。**検索・タグ・置き場所**。オフライン下書き。病害虫・薬剤の**構造化フィールド**。盆栽向け**テンプレート**。 |
...
植物詳細の **AI診断・ケア提案** は Edge Functions（`ai-diagnose`, `care-recommendations`）を呼び出します。ボタンがエラーになる場合はプロジェクトに **デプロイ済みか**確認してください: `supabase functions deploy ai-diagnose` と `care-recommendations`（またはダッシュボードから同等のデプロイ）。
