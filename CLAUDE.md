...
**plant** は植物の登録・世話記録・写真・AI による状態の可能性提示（決めつけない）を扱うモバイルアプリです。**対象は盆栽・ビカクシダ（コウモリラン）など嗜好が強い植物から一般の観葉植物まで**幅広く想定し、UI・文言は特定ジャンルに偏らないようにする。**セキュリティとプライバシーを最優先**し、クライアントに AI 系 API キーや `service_role` を置かない方針です。
...
- 機能の優先度・見送りの一覧は [README.md](./README.md) の「機能の洗い出し」を参照。
